...
interface ProductsBrowserProps {
  /**
   * Catalogue to browse. Fetched server-side by `app/produkty/page.tsx`
   * and handed down — this component is client-side (filter/sort state)
   * and must never touch the Shopify layer directly, which holds the
   * private token.
   */
  products: Product[];
}
...
export function ProductsBrowser({ products }: ProductsBrowserProps) {
  ...
  const filtered = useMemo(
    () => applyFilters(products, category, drawerFilters, sortBy),
    [products, category, drawerFilters, sortBy]
  );
    ...
    <>
      ...
      <section className="px-5 lg:px-14 pt-10 lg:pt-16 pb-8 lg:pb-12 border-b border-line">
        <div className="flex items-end justify-between gap-10">
          <div>
            ...
            >
              {l.eyebrow} · {l.count(products.length)}
            </p>
            <h1
      ...
      <section className="px-5 lg:px-14 pt-8 lg:pt-12 pb-24 lg:pb-32">
        ...
        {filtered.length > 0 ? (
          ...
          /* ── Empty catalogue ──
             Distinct from the "no filter matches" state below: nothing is
             published yet, so offering a "clear filters" CTA would be
             misleading. */
          <div className="flex flex-col items-center justify-center py-28 text-center">
            <p
              ...
              style={{ fontFamily: "var(--font-display)" }}
            >
              {l.emptyCatalog.heading}
            </p>
            <p className="text-[14px] text-muted leading-[1.5] max-w-[320px]">
              {l.emptyCatalog.body}
            </p>
          </div>
        ) : (
